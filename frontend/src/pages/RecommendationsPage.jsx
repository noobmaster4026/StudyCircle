...
export default function RecommendationsPage() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setFormSubjects(profileRes.data.subjects?.join(', ') || '');
      setFormGoals(profileRes.data.goals?.join(', ')       || '');
      if (profileRes.data.deadlines?.length > 0) {
        setFormDeadlines(profileRes.data.deadlines.map(d => ({
          subject: d.subject,
          topic:   d.topic,
  ...
  const handleFeedback = async (id, type) => {
    ...
    try {
      ...
      setRecommendations(prev => prev.map(r => {
        ...
        return {
          ...r,
          thumbsUp:   type === 'up'   ? r.thumbsUp   + 1 : r.thumbsUp,
          thumbsDown: type === 'down' ? r.thumbsDown + 1 : r.thumbsDown,
        };
      }));
    } catch (err) {
      ...
      {loading ? (
        ...
        <div className="rec-grid">
          {filtered.map(rec => {
            ...
            const total    = rec.thumbsUp + rec.thumbsDown;
            const likeRatio = total > 0 ? Math.round((rec.thumbsUp / total) * 100) : null;
