...
router.post('/seed', async (req, res) => {
  try {
    ...
    if (existing > 0) {
      return res.json({ message: `Already seeded with ${existing} recommendations` });
    }
    ...
    const samples = [
      ...
      // Tutors
      { type: 'tutor', title: 'Ahmed R. — Algorithms Specialist',  subject: 'Algorithms',  tags: ['algorithms', 'competitive', 'expert'],  difficulty: 'advanced',     description: 'PhD student specializing in competitive programming. 4.9★ rating from 120 sessions.',  score: 9 },
    ...
    await Recommendation.insertMany(samples);
    res.json({ message: `Seeded ${samples.length} recommendations successfully` });
  } catch (err) {
